...
class Station:
    ...
    def __init__(self, player, station_id=0, debug=False):
        ...
        if player is None:
            raise Error(
                "A player must be specified when creating a new station")
        else:
            self.player = player
    ...
    def next(self):
        ...
        params = urllib.urlencode(
            {"cover_size": "m", "format": "aac", "buffer": 0})
        conn.request(
            "POST", self.get_station_path() + "/next", params, self.headers)
    ...
    # Also sets the `current_track` property with the current track
    # information.
    def play_next(self):
        # get the next track
    ...
    def query_station(self, query):
        ...
        # create request body
        conn.request(
            "GET", "/api/1/search/station?query=" + query, None, self.headers)
    ...
    def vote(self, song_id, up):
        ...
        url = "/api/1/station/{0}/song/{1}/vote/{2}".format(
            self.station_id, song_id, direction)
